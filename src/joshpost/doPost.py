...
def find_pop_correlation(norm_corr,time):
  """ time = np.na sdn;
   ...
   time = find_max_time(peak_times,norm_corr,end_time)"""
  return norm_corr[time]

##Method only checks up the first auto corr peak and uses that the window is set for only one phase shift in the auto
...
##    plt.show()
    ...
    phase_lag, peak_time  = find_phase_lag(cross_correlation,auto_cross_correlation1)
    pop_correlation = find_pop_correlation(normalized_cross_correlation,peak_time)
