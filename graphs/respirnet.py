...
def dist_prebot(n, pTypes, pI, gE, gI):
    '''

        This Method creates neuron edges based on the distance between neuron
        ...
        pTypes: probability of each node type

        pI: probability that a neuron is inhibitory

        gE: Max conductance of excitatory connections

        gI: Max conductance of inhibitory connections
    ...
    for e in edges:
        d = math.fabs(e[0] - e[1])/n
        p = c*math.exp(-d**2/sigma**2)
...
def dist_block_prebot(n, pTypes, pI, gE, gI):
    '''
    ...
    #create the graph and assign the nodes thier arbitrary chosen attributes
    ...
    for e in edges:
        ...
        v = graph.node[e[1]]
        if u['inh'] == 1:
            if u['respir_area'] != v['respir_area']:
                c = .9
                sigma = .58
                d = math.fabs(e[0] - e[1])/n
                p = c*math.exp(-d**2/sigma**2)
        ...
        if u['inh'] == 0:
            if u['respir_area'] == v['respir_area']:
                c = .9
                sigma = .024
                d = math.fabs(e[0] - e[1])/n #maybe 2n
                p = c*math.exp(-d**2/sigma**2)
	    else:
		p = 0	
        if random.random() < p:
